...
sys_write(void)
{
    ...
    char buf[512];
    int tot = 0;
    klog(LOG_LEVEL_DEBUG, "sys_write: fd=%d n=%d", fd, n);
    while (n > 0)
    {
...
sys_exit(void)
{
    int status;
    argint(0, &status);
    klog(LOG_LEVEL_INFO, "sys_exit: pid=%d status=%d", myproc()->pid, status);
    exit(status);
...
{
    int r = fork();
    klog(LOG_LEVEL_INFO, "sys_fork: parent pid=%d -> ret=%d", myproc()->pid, r);
    return r;
}
...
sys_wait(void)
{
    ...
    argaddr(0, &addr);
    int r = wait(addr);
    klog(LOG_LEVEL_INFO, "sys_wait: pid=%d ret=%d", myproc()->pid, r);
    return r;
}
...
sys_klog(void)
{
    ...
    if (copyout_user(myproc()->pagetable, ubuf, tmp, r) < 0)
        return -1;
    klog(LOG_LEVEL_DEBUG, "sys_klog: pid=%d read=%d", myproc()->pid, r);
    return r;
}
