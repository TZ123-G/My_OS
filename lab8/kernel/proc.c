...
found:
    ...
    // 记录进程分配日志（DEBUG）
    klog(LOG_LEVEL_DEBUG, "allocproc: pid=%d", p->pid);
...
int fork(void)
{
    ...
    if ((np = allocproc()) == 0)
    {
        klog(LOG_LEVEL_ERROR, "fork: allocproc failed (parent pid=%d)", p ? p->pid : -1);
        return -1;
    }
    ...
    {
        klog(LOG_LEVEL_ERROR, "fork: create_pagetable failed (child pid=%d)", np->pid);
        freeproc(np);
        release(&np->lock);
        return -1;
    }
    ...
    {
        klog(LOG_LEVEL_ERROR, "fork: copy_pagetable_mapping failed (parent pid=%d child pid=%d)", p->pid, np->pid);
        freeproc(np);
        release(&np->lock);
        return -1;
    }
    ...
    *(np->trapframe) = *(p->trapframe);
    ...
    // 记录成功 fork（INFO）
    klog(LOG_LEVEL_INFO, "fork: parent pid=%d -> child pid=%d", p->pid, np->pid);
...
void exit(int status)
{
    ...
    // 记录进程退出（WARN）
    klog(LOG_LEVEL_WARN, "exit: pid=%d status=%d", p->pid, status);
        ...
        for (pp = proc; pp < &proc[NPROC]; pp++)
        {
            if (pp->parent == p)
            {
                ...
                if (pp->state == ZOMBIE)
                {
                    ...
                    // 记录 wait 收集子进程（INFO）
                    klog(LOG_LEVEL_INFO, "wait: parent pid=%d collected child pid=%d status=%d", p->pid, pp->pid, pp->xstate);
                    freeproc(pp);
                    release(&pp->lock);
                    release(&wait_lock);
                    return pid;
